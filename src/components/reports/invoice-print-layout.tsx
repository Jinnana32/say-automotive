...
export function InvoicePrintLayout({
  ...
  const sourceLabel = invoice.jobOrderNumber ?? invoice.saleNumber ?? "Direct sale";
  const paidTotals = invoice.payments.reduce((sum, payment) => sum + payment.amount, 0);
      ...
      <section className="report-section-keep mt-5 grid gap-x-8 gap-y-2 sm:grid-cols-2">
        <MetadataColumn
          items={[
            ...
            { label: "Contact Number", value: invoice.customerContactNumber || "—" },
      ...
      <section className="report-section-keep mt-3 grid gap-x-8 gap-y-2 sm:grid-cols-2">
        <MetadataColumn
          items={[
            ...
            { label: "Released", value: invoice.releasedAt ? formatDateTime(invoice.releasedAt) : "—" },
          ]}
        />
        <MetadataColumn
          items={[
          ...
          <table className="w-full border-collapse text-[11px]">
            <thead className="bg-slate-100">
              <tr>
                ...
                <th className="w-16 px-2 py-1.5 text-right font-semibold">Qty</th>
              ...
              <table className="w-full border-collapse text-[11px]">
                <thead className="bg-slate-100">
                  <tr>
                    ...
                    <th className="w-24 px-2 py-1.5 text-right font-semibold">Amount (₱)</th>
                  </tr>
                </thead>
                <tbody>
                  {invoice.payments.length > 0 ? (
                    invoice.payments.map((payment) => (
                      <tr key={payment.id} className="report-row-avoid border-t border-slate-200">
