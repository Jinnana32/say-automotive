...
begin
  ...
  if v_item.is_additional and v_item.approval_status <> 'approved' then
...
begin
  ...
  if v_net_used <= 0 then
    raise exception 'No used quantity is available to return.';
  end if;

  if p_quantity > v_net_used then
    raise exception 'Cannot return more than the currently used quantity.';
  end if;
  ...
  set
    usage_status = case
      when v_net_used - p_quantity = 0 then 'returned'
      else 'used'
    end,
    updated_at = timezone('utc', now())
  where id = v_item.id;
