...
begin
  ...
  if v_job_order.status not in ('in_progress', 'completed', 'ready_for_billing', 'paid') then
    raise exception 'Only in-progress, completed, billing-ready, or paid job orders can be released.';
  end if;
