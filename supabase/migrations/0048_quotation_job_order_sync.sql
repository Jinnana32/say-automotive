create table if not exists public.quotation_revisions (
  ...
  previous_total_amount numeric(14,4) not null default 0,
  new_total_amount numeric(14,4) not null default 0,
  change_summary jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default timezone('utc', now())
);

create index if not exists quotation_revisions_quotation_id_idx
  on public.quotation_revisions (quotation_id);

create index if not exists quotation_revisions_job_order_id_idx
  on public.quotation_revisions (job_order_id);

alter table public.quotation_revisions enable row level security;

drop policy if exists quotation_revisions_select_branch on public.quotation_revisions;
create policy quotation_revisions_select_branch
...
create or replace function public.delete_job_order_item_impl(
...
begin
  ...
  if v_job_order.status in ('completed', 'ready_for_billing', 'paid', 'released', 'cancelled') then
    raise exception 'Work items cannot be removed at this job order status.';
  end if;

  if v_item.usage_status = 'used' then
    raise exception 'Used work items cannot be removed.';
  end if;
  ...
  if v_item.item_type = 'product' and v_item.product_id is not null then
    ...
    if v_net_used > 0 then
      raise exception 'Work items with recorded part usage cannot be removed.';
    end if;
  end if;

  v_quotation_item_id := v_item.source_quotation_item_id;

  delete from public.job_order_items
  where id = p_job_order_item_id;
...
create or replace function public.sync_quotation_with_job_order(
  p_quotation_id uuid,
  p_nature_of_repair text,
  p_inspection_notes text,
  p_subtotal numeric,
...
begin
  perform public.assert_has_any_staff_role(
    ...
    'Only service-administration roles can revise quotations.'
  );
  ...
  perform public.assert_branch_access(
    v_branch_id,
    'You do not have access to revise quotations for this branch.'
  );

  return public.sync_quotation_with_job_order_impl(
    p_quotation_id,
    p_nature_of_repair,
    p_inspection_notes,
    p_subtotal,
...
revoke all on function public.delete_job_order_item_impl(uuid) from public, anon, authenticated;
revoke all on function public.sync_quotation_with_job_order_impl(
  uuid,
  text,
  text,
  numeric,
...
revoke all on function public.sync_quotation_with_job_order(
  uuid,
  text,
  text,
  numeric,
...
grant execute on function public.sync_quotation_with_job_order(
  uuid,
  text,
  text,
  numeric,
